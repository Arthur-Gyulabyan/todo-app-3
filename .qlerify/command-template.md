...
4. **Implementation**
   - Assume that Entity classes (e.g., `new Todo(...)` or `todo.update(...)`) expect objects whose property names and casing already match the OpenAPI specification's schemas. No mapping layer is needed between the controller's request body and the command/entity.
